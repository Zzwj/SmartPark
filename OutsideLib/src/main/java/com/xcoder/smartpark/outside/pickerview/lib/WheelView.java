package com.xcoder.smartpark.outside.pickerview.lib;
...
import com.xcoder.smartpark.outside.pickerview.adapter.WheelAdapter;
import com.xcoder.smartpark.outside.pickerview.listener.OnItemSelectedListener;
import com.xcoder.smartpark.outside.pickerview.model.IPickerViewData;
...
public class WheelView extends View {
    ...
    int textLabelSize=34;//选项中label的文字大小-注：写的字体大小是xml里面3倍--xcoder_xz改
    ...
    // 条目间距倍数
    static final float lineSpacingMultiplier = 2.1F;
    boolean isLoop;
...
//            paintCenterLabelText.getTextBounds(s1, 0, s1.length(), rect);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //单位的Label
        if (label != null) {
            //对label的文字进行单独的设置，---xcoder_xz改
            Paint  paintCenterLabelText  =new Paint(paintCenterText);
            paintCenterLabelText.setTextSize(textLabelSize);

            int drawRightContentStart = (int) (measuredWidth - (int) (getTextWidth(paintCenterLabelText, label))*1.2);
            //int drawRightContentStart = measuredWidth - getTextWidth(paintCenterText, label);
            //靠右并留出空隙
            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET, centerY, paintCenterLabelText);
