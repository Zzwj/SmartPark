...
public class MapUtils {
	...
	public synchronized static String getLocationStr(AMapLocation location) {
		...
		if (location.getErrorCode() == 0) {
			...
			locationModel.setAccuracy(location.getAccuracy() + "米");
			...
			if (location.getProvider().equalsIgnoreCase(
				...
				locationModel.setArea(location.getDistrict());
				...
				locationModel.setTime(formatUTC(location.getTime(),
						"yyyy-MM-dd HH:mm:ss:sss"));
				// 存用户定位信息
				ConfigSP.setLocation(locationModel);
			}
		} else {
			// 定位失败
			sb.append("定位失败" + "\n");
